...
style_line blue bold "Installing Nginx and PHP..."
apt-get install -y nginx php5-fpm > /dev/null

# Link modified Nginx conf
style_line blue bold "Linking modified Nginx conf..."
rm -rf /etc/nginx/nginx.conf
ln -fs $PROJECT_ROOT/conf/nginx.conf /etc/nginx/nginx.conf

# Install MySQL
style_line blue bold "Installing MySQL..."
apt-get -q -y install mysql-server mysql-client  > /dev/null

# Install additional required packages
style_line blue bold "Installing PHP and additional packages..."
...
style_line blue bold "Installing Redis..."
...
# Install PHPMyAdmin
style_line blue bold "Installing PHPMyAdmin..."
